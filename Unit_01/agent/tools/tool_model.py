...
class WebSearchSummarizeService(object):
    ...
    async def summarize_service(self, text: list[str]):
        chain = await self.get_chain()
        res = await chain.abatch([{"input": input_text} for input_text in text])
        return res
# {"input": text}
...
if __name__ == "__main__":
    ...
    response = asyncio.run(model.summarize_service(["""
